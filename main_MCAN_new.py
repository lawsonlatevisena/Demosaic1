...
parser.add_argument("--nEpochs", type=int, default=8500, help="number of epochs to train for")
...
parser.add_argument("--train_dir", default="/home/llawson/MCAN/Mosaic-Convolution-Attention-Network-for-Demosaicing-Multispectral-Filter-Array-Images-main/CAVE_dataset/new_val", type=str, help="path to train dataset")
...
'''
def train(training_data_loader, optimizer, model, criterion, criterion1, epoch, num_epochs, save_images_interval=100):
    lr = adjust_learning_rate(optimizer, epoch-1)
    ...
    for batch_idx, batch in enumerate(train_bar):
        ...
        # Save images at a specific interval
        if batch_idx % save_images_interval == 0:
            save_images(input_raw, HR_4x, label_x4, epoch, batch_idx, phase='train')
...
def test(testing_data_loader, optimizer, model, criterion, criterion1, epoch, num_epochs):
    ...
    return test_results
'''
def test(testing_data_loader, optimizer, model, criterion, criterion1, epoch, num_epochs, save_images_interval=100):
    test_bar = tqdm(testing_data_loader)
    test_results = {'batch_sizes': 0, 'psnr': 0, 'mse': 0}
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_bar):
            ...
            # Save images at a specific interval
            if batch_idx % save_images_interval == 0:
                save_images(input_raw, HR_4x, label_x4, epoch, batch_idx, phase='test')
