...
class Net(nn.Module):
    def __init__(self):
        ...
        # Denoising layers
        self.denoising_conv = nn.Conv2d(in_channels=16, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.denoising_relu = nn.LeakyReLU(0.2, inplace=True)
        self.denoising_output = nn.Conv2d(in_channels=64, out_channels=16, kernel_size=3, stride=1, padding=1)
        ...
        self.convt_F2 = self.make_layer(_Conv_attention_Block)
        self.convt_br1_back = self.make_layer(branch_block_back)
        self.P2W = Pos2Weight(outC=self.outC, inC=1)
       # self.P2W = Pos2Weight(outC=self.outC)
    ...
    def repeat_y(self, y):
        scale_int = math.ceil(self.scale)
        N, C, H, W = y.size()  # Extract dimensions of the tensor
        y = y.view(N, C, H, 1, W, 1)  # Add two new dimensions for repetition

    # Repeat the tensor in height and width dimensions
        y = torch.cat([y] * scale_int, 3)
        y = torch.cat([y] * scale_int, 5).permute(0, 3, 5, 1, 2, 4)

        return y.contiguous().view(-1, C, H, W)  # Return the repeated tensor
    ...
    def forward(self, data, pos_mat):
        ...
        # Denoising mechanism
        noisy_x = self.denoising_conv(x)
        noisy_x = self.denoising_relu(noisy_x)
        estimated_noise = self.denoising_output(noisy_x)
        denoised_x = x - estimated_noise

        WB_norelu = self.WB_Conv(denoised_x)
